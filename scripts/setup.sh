...
for i in $(seq 1 30); do
    ...
    if [ "$HTTP_CODE" = "200" ]; then
        ...
        echo ""
        echo "  ── DNS Setup ──────────────────────────────────────"
        echo "  At your domain registrar, point nameservers to this server:"
        echo "    1. Set custom nameservers: ns1.$DOMAIN, ns2.$DOMAIN"
        echo "    2. Create glue records:"
        echo "       ns1.$DOMAIN -> $SERVER_IP"
        echo "       ns2.$DOMAIN -> $SERVER_IP"
        echo ""
        echo "  Namecheap: Domain List -> Manage -> Custom DNS + Advanced DNS"
        echo "  Cloudflare: Not applicable (use Namecheap/Porkbun/etc)"
        echo "  Porkbun: DNS -> Edit Nameservers + Glue Records"
        echo ""
        echo "  ── Connect Claude Code (MCP) ──────────────────────"
        echo "  Build the MCP server (needs Rust on your local machine):"
        echo "    cargo install --path mcp-server"
        echo ""
        ...
        echo "    }"
        echo ""
        echo "  Restart Claude Code. 15 tools will be available:"
        echo "  deploy_preview, teardown, list_deployments, get_logs,"
        echo "  map_custom_domain, promote, plan_deploy, and more."
        echo ""
        echo "  ── CLI ────────────────────────────────────────────"
        ...
        echo ""
        echo "  ── Features ───────────────────────────────────────"
        echo "  Subdomain deploy:  repo-branch.$DOMAIN"
        echo "  Path routing:      $DOMAIN/client/staging"
        echo "  Custom domains:    client.com -> your deployment"
        echo "  GitHub webhooks:   Auto-deploy on push"
        echo "  Plan/Apply:        Safe agent-driven deploys"
        echo ""
        echo "  Setup log: $LOG"
        echo ""
